...
map_df61_nested %>% 
  unnest() %>% 
  select(x, alpha1, alpha2, alpha3, alpha4, pE1, pE2, pE3, pE4, pS, n1_response, n2_response, n3_response, n4_response) %>% 
  distinct() %>% 
  filter(x == 0.15) %>% 
  head()
```
...
Using the direct calculation method, it is possible to determine that response rates in each of the 4 interim analyses of 10, 8, 8, and 4 will satisfy the conditions. So, we can use these as the futility cutoffs.
...
dat = tibble(means = c(sum(rbeta(n_sim, shape1 = 10.5, shape2 = 11.5) > (rbeta(n_sim, shape1 = 25, shape2 = 75) + x)) / n_sim,
                      ...
                      sum(rbeta(n_sim, shape1 = 7.5, shape2 = 14.5) > (rbeta(n_sim, shape1 = 25, shape2 = 75) + x)) / n_sim,
                      # sum(rbeta(n_sim, shape1 = 8.5, shape2 = 13.5) > (rbeta(n_sim, shape1 = 25, shape2 = 75) + x)) / n_sim,
                      # sum(rbeta(n_sim, shape1 = 7.5, shape2 = 36.5) > (rbeta(n_sim, shape1 = 25, shape2 = 75) + x)) / n_sim,
                      sum(rbeta(n_sim, shape1 = 4.5, shape2 = 7.5) > (rbeta(n_sim, shape1 = 25, shape2 = 75) + x)) / n_sim,
                      sum(rbeta(n_sim, shape1 = 3.5, shape2 = 8.5) > (rbeta(n_sim, shape1 = 25, shape2 = 75) + x)) / n_sim))
dat
```
